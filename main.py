...
def get_market_cap(symbol):
    """Get market cap in USD. Cached for 1 hour."""
    if symbol in mcap_cache:
        cached_time, cached_val = mcap_cache[symbol]
        if time.time() - cached_time < 3600:
            return cached_val
    cg_map = {sym: cg for cg, sym in ALL_COINS}
    cg_id  = cg_map.get(symbol)
    if not cg_id: return 0
    try:
        resp = requests.get(
        ...
        data = resp.json()
        if data:
            mcap = data[0].get("market_cap", 0) or 0
            mcap_cache[symbol] = (time.time(), mcap)
            return mcap
    except:
        pass
    return 0

def get_hold_time(symbol):
    """
    Dynamic max hold time based on market cap.
    Large cap  (>$10B):  24 hours
    Mid cap    ($1B-10B): 4 hours
    Small cap  ($100M-1B): 2 hours
    Micro cap  (<$100M):  90 minutes
    """
    mcap = get_market_cap(symbol)
    if mcap >= 10_000_000_000:
        return 1440, "large cap"    # 24 hours
    elif mcap >= 1_000_000_000:
        return 240, "mid cap"       # 4 hours
    elif mcap >= 100_000_000:
        return 120, "small cap"     # 2 hours
    else:
        return 60, "micro cap"      # 60 minutes

def get_targets(symbol):
    """
    Dynamic take profit and stop loss based on market cap.
    Large cap:  TP +6%  SL -3%
    Mid cap:    TP +5%  SL -2.5%
    Small cap:  TP +4%  SL -2%
    Micro cap:  TP +3%  SL -1.5%
    """
    mcap = get_market_cap(symbol)
    if mcap >= 10_000_000_000:
        return 6.0, 3.0
        ...
        try:
            if open_positions:
                ...
                for symbol, pos in list(open_positions.items()):
                    ...
                    # Dynamic targets and hold time based on market cap
                    tp_pct, sl_pct = get_targets(symbol)
                    max_mins, cap_type = get_hold_time(symbol)
                    reason = None
                    if pct >= tp_pct:       reason = f"✅ TAKE PROFIT +{pct:.1f}% ({cap_type})"
                    elif pct <= -sl_pct:    reason = f"❌ STOP LOSS {pct:.1f}% ({cap_type})"
                    elif mins >= max_mins:  reason = f"⏰ TIME EXIT {pct:+.1f}% after {mins:.0f}m ({cap_type})"
                    if reason:
            ...
            for cg_id, symbol in SMALL_CAPS:
                ...
                try:
                    ...
                    if sig != "NEUTRAL" and conf >= MIN_CONFIDENCE:
                        # Dynamic targets based on market cap
                        tp_pct, sl_pct = get_targets(symbol)
                        max_mins, cap_type = get_hold_time(symbol)
                        target = usd*(1+tp_pct/100) if sig=="LONG" else usd*(1-tp_pct/100)
                        stop   = usd*(1-sl_pct/100) if sig=="LONG" else usd*(1+sl_pct/100)
                        ...
                        if order:
                            ...
                            send_telegram(
                                ...
                                f"{sig} @ {conf}% | {cap_type}\n"
                                f"💰 Entry: ${usd:.4f}\n"
                                f"🎯 Target: ${target:.4f} (+{tp_pct}%)\n"
                                f"🛑 Stop: ${stop:.4f} (-{sl_pct}%)\n"
