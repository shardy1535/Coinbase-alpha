...
def get_binance_prices() -> dict:
    ...
    try:
        ...
        # Fetch specific symbols only — faster and avoids full list issues
        symbols_to_fetch = []
        for sym in wanted:
            bn = "XBT" if sym == "BTC" else sym
            symbols_to_fetch.append(f'"{bn}USDT"')

        symbols_json = "[" + ",".join(symbols_to_fetch) + "]"
        resp = requests.get(
        ...
        if isinstance(data, list):
            for item in data:
                ...
                if ticker.endswith("USDT") and price:
                    sym = ticker[:-4]
                    if sym in wanted:
                        prices[sym] = float(price)
