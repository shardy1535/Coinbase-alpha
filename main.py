...
SCAN_SECS       = int(os.getenv("SCAN_SECONDS", "60"))        # scan every 60s
...
def get_all_prices():
    """Fetch current prices for all coins in one API call."""
    try:
        ids = ",".join(cg for cg, _ in ALL_CG_IDS)
        resp = requests.get(
        ...
        data = resp.json()
        # Return as symbol -> price dict
        result = {}
        for cg_id, symbol in ALL_CG_IDS:
...
def get_momentum(symbol: str) -> float | None:
    """
    Calculate % price change over last MOMENTUM_WINDOW minutes.
    ...
    now = time.time()
    window_secs = MOMENTUM_WINDOW * 60

    # Get oldest price within the window
    window_prices = [(t, p) for t, p in history if now - t <= window_secs]
