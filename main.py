...
def get_binance_prices() -> dict:
    """Get real-time prices from Binance — fetch all tickers and filter."""
    try:
        ...
        # Parse manually to avoid type issues
        import json as _json
        data = _json.loads(raw)

        if not isinstance(data, list):
            print(f"  Binance returned non-list: {type(data)}")
            return {}

        for item in data:
            ticker = item.get("symbol", "")
            price  = item.get("price", "")
            if not ticker.endswith("USDT") or not price:
                continue
            sym = ticker[:-4]
            if sym in wanted:
                try:
