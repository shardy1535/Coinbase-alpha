...
def get_coinbase_prices() -> dict:
    """Get prices from Coinbase public ticker — no auth needed."""
    try:
        prices = {}
        symbols = [sym for _, sym in COINBASE_COINS]
        # Use public exchange rates endpoint
        for sym in symbols[:10]:  # batch to avoid rate limits
            try:
                resp = requests.get(
...
def get_binance_prices() -> dict:
    ...
    try:
        ...
        for sym in wanted:
            ...
            symbols.append(f"{bn}USDT")

        # Fetch all at once using bookTicker
        resp = requests.get(
        ...
        if not isinstance(data, list):
            print(f"  Binance unexpected response type: {type(data)}")
            return {}
