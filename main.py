...
def get_binance_prices() -> dict:
    """Get extra coin prices from CoinGecko to supplement Kraken."""
    try:
        wanted  = {sym for _, sym in COINBASE_COINS + KRAKEN_COINS}
        cg_map  = {sym: cg for cg, sym in COINBASE_COINS + KRAKEN_COINS}
        # Only fetch coins not already covered by Kraken
        kraken_syms = {sym for _, sym in KRAKEN_COINS}
        extra_syms  = [s for s in wanted if s not in kraken_syms]
        extra_ids   = [cg_map[s] for s in extra_syms if s in cg_map]
        ...
        resp = requests.get(
            ...
            params={"ids": ",".join(extra_ids[:20]), "vs_currencies": "usd"},
            timeout=8
        )
        data   = resp.json()
        prices = {}
        for cg_id, symbol in COINBASE_COINS:
            if symbol not in kraken_syms:
                price = data.get(cg_id, {}).get("usd")
                if price:
                    prices[symbol] = price
        return prices
    except Exception as e:
        print(f"  Extra prices error: {e}")
        return {}
