"""
COINBASE ALPHA SUITE — WITH BUY/SELL + DAILY SUMMARY
"""
...
daily_pnl       = 0.0
daily_trades_log = []  # list of {symbol, pnl, side}
...
def execute_buy(symbol, size_usd, price):
    global daily_pnl
    if KILL_SWITCH:
        return None
    ...
    if symbol in open_positions:
        return None
    print(f"  📝 PAPER BUY: {symbol} ${size_usd} @ ${price:.4f}")
    return {"order_id": f"PAPER-{symbol}-{int(time.time())}", "paper": True}

def execute_sell(symbol, size_usd, price, reason):
    print(f"  📝 PAPER SELL: {symbol} @ ${price:.4f} — {reason}")
    return {"order_id": f"PAPER-SELL-{symbol}-{int(time.time())}", "paper": True}
...
def run_position_monitor():
    global open_positions, daily_pnl, daily_trades_log
        ...
        try:
            if open_positions:
                ...
                for symbol, pos in open_positions.items():
                    ...
                    size    = pos["size_usd"]
                    print(f"  {symbol}: ${entry:.4f}→${current:.4f} ({pct_chg:+.2f}%)")
                    ...
                    if reason:
                        order = execute_sell(symbol, size, current, reason)
                        if order:
                            pnl = size * (pct_chg / 100)
                            daily_pnl += pnl
                            daily_trades_log.append({"symbol": symbol, "pnl": pnl, "side": pos["side"]})
...
def run_daily_summary():
    global daily_pnl, daily_trades_log
    last_date = None
    ...
    while True:
        try:
            ...
            if now.hour == 0 and now.minute == 0 and now.date() != last_date:
                ...
                if worst:
                    msg += f"\n💀 Worst trade: {worst['symbol']} *${worst['pnl']:.2f}*"
                if not daily_trades_log:
                    msg += "\n_No trades today — waiting for signals_"
        ...
        try:
            ...
            for cg_id, symbol in ALL_COINS:
                ...
                if best:
                    ...
                    print(f"  {symbol}: 🚨 ARB +{best['net_pct']:.3f}%")
    ...
    while True:
        try:
            ...
            for cg_id, symbol in SMALL_CAPS:
                ...
                # Only analyze coins with momentum
                ...
                prompt = (
                    f"Crypto momentum trade analysis for {symbol}/USD:\n"
                    f"Price: ${usd:.4f}\n"
                    f"1h: {ch1h:.2f}%  24h: {ch24:.2f}%\n"
                    f"Fear & Greed: {fg_val}/100 ({fg_lb})\n\n"
                    f"Is this a good entry for a 5-7% gain?\n"
                    f"Respond ONLY as JSON:\n"
                    f'ONLY output: {{"signal":"LONG or SHORT or NEUTRAL","confidence":0-100,"reasoning":"one sentence"}}'
                )
                ...
                try:
                    ...
                    if sig != "NEUTRAL" and conf >= MIN_CONFIDENCE:
                        ...
                        order = execute_buy(symbol, MAX_POSITION_USD, usd)
                        if order:
                            ...
                            send_telegram(
                                ...
                                f"💵 Size: ${MAX_POSITION_USD}\n\n"
                                f"📝 {reason}\n\n"
                                f"⏰ {datetime.now(timezone.utc).strftime('%H:%M UTC')}\n"
                                f"_Paper trade mode_"
                            )
                except Exception as e:
...
╚══════════════════════════════════════════════════╝
  ...
  🔄 Arb Scanner       — {len(ALL_COINS)} coins, 3 exchanges
  🤖 Signal Analyzer   — {len(SMALL_CAPS)} small caps
  📊 Position Monitor  — auto sell +{TAKE_PROFIT_PCT}% / -{STOP_LOSS_PCT}%
  🐋 Whale Watcher     — on-chain moves
  📅 Daily Summary     — midnight UTC recap

  Min confidence: {MIN_CONFIDENCE}%  |  Max position: ${MAX_POSITION_USD}
...
""")
    ...
    send_telegram(
        ...
        f"💵 Max position: ${MAX_POSITION_USD}\n"
        f"📅 Daily summary: midnight UTC\n\n"
        f"_Paper trade mode — watching before going live_\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%H:%M UTC')}"
    )
    ...
    while True:
        ...
        print(f"  ♻ {len(alive)}/5 modules | positions: {pos_str} | P&L today: ${daily_pnl:+.2f}")
