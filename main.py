...
def get_kraken_prices() -> dict:
    """Get real-time prices from Kraken public API — updates every second."""
    try:
        # Get all ticker data in one call
        pairs = []
        ...
        resp = requests.get(
            ...
            params={"pair": ",".join(pairs[:20])},  # batch 20 at a time
            timeout=8
        )
        ...
        for pair_name, data in result.items():
            ...
            for _, symbol in KRAKEN_COINS:
                kb = "XBT" if symbol == "BTC" else symbol
                expected = f"X{kb}ZUSD" if kb == "XBT" else f"{kb}USD"
                if pair_name == expected or pair_name.startswith(kb):
...
def get_coinbase_prices() -> dict:
    """Get real-time prices from Coinbase public API — updates every second."""
    try:
        prices = {}
        # Coinbase best bid/ask endpoint — real time
        symbols = [sym for _, sym in COINBASE_COINS]
        product_ids = [f"{sym}-USD" for sym in symbols]
        
        resp = requests.get(
            "https://api.coinbase.com/api/v3/brokerage/best_bid_ask",
            params={"product_ids": ",".join(product_ids[:20])},
            timeout=8
        )
        pricebooks = resp.json().get("pricebooks", [])
        for pb in pricebooks:
            product = pb.get("product_id", "")
            symbol  = product.replace("-USD", "").replace("-USDC", "")
            bids    = pb.get("bids", [])
            asks    = pb.get("asks", [])
            if bids and asks:
...
def get_binance_prices() -> dict:
    """Get real-time prices from Binance — fastest public feed."""
    try:
        resp = requests.get(
...
def get_momentum(symbol: str) -> float | None:
    ...
    # Reject if old price is too recent (poor window coverage)
    if now - closest[0] < (MOMENTUM_WINDOW * 60 * 0.7):
        return None

    return ((current_p - closest[1]) / closest[1]) * 100
    ...
    for coins, positions, exchange, order_fn, balance in [
        ...
        for cg_id, symbol in coins:
            ...
            if order:
                ...
                positions[symbol] = {
                    "entry": price, "size_usd": POSITION_USD,
                    "target": target, "stop": stop,
                    "opened_at": time.time(), "order_id": order,
                }
...
""")
    ...
    while True:
        try:
            ...
            if movers:
                top = " | ".join(f"{s}:{m:+.3f}%" for s,m in movers[:5])
                print(f"  Top movers: {top}")
            else:
                print(f"  No movement detected yet — prices may be identical")
                # Show sample price history length
                for sym in ["BTC","ETH","SOL"]:
                    h = price_history.get(sym, [])
                    if h:
                        oldest = h[0][1]
                        newest = h[-1][1]
                        print(f"  {sym}: {len(h)} snapshots, ${oldest:.2f}→${newest:.2f}")
