...
# Install everything before anything else runs
pkgs = ["requests", "PyJWT", "cryptography", "python-dotenv"]
for pkg in pkgs:
...
def discover_verified_pairs():
    ...
    try:
        resp = requests.get(
            "https://api.coinbase.com/api/v3/brokerage/products", timeout=15
        )
        for p in resp.json().get("products", []):
            parts = p.get("product_id","").split("-")
            if len(parts) == 2 and parts[1] in ("USD","USDC"):
                coinbase_available.add(parts[0])
        print(f"  Coinbase: {len(coinbase_available)} USD/USDC pairs")
    except Exception as e:
        print(f"  Coinbase discovery error: {e}")
