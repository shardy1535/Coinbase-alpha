...
def check_positions(positions, exchange, place_order_fn):
    ...
    for symbol, pos in list(positions.items()):
        ...
        # Fixed targets: 5% profit, 1% stop loss
        tp, sl, max_mins = 5.0, 1.0, 120
...
def analyze_and_trade(coin_list, positions, exchange, place_order_fn, balance_fn, prices, fg_val, fg_lb):
    ...
    for cg_id, symbol in coin_list:
        ...
        # Only trade coins with 1-2% move in last hour — catching start of momentum
        if ch1h < 1.0 or ch1h > 8.0:
            # Skip if less than 1% (no momentum) or more than 8% (already ran too much)
            print(f"  [{exchange}] {symbol}: 1h={ch1h:+.1f}% — skip")
            continue
        print(f"  [{exchange}] 🎯 {symbol}: momentum detected {ch1h:+.1f}% in 1h")
        ...
        # Fixed targets: 5% profit, 1% stop, 2hr max hold
        tp, sl, pos_size, max_mins = 5.0, 1.0, 100.0, 120
