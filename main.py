...
for pkg in ["requests", "PyJWT", "cryptography"]:
    try:
        __import__(pkg.lower().replace("pyjwt", "jwt"))
    except ImportError:
...
try:
    ...
    JWT_AVAILABLE = False
    print("WARNING: PyJWT not installed — Coinbase JWT auth unavailable")
...
# ─────────────────────────────────────────
# ALL COINS — bot auto-verifies on startup
# which ones trade on BOTH exchanges
# ─────────────────────────────────────────
ALL_POSSIBLE_COINS = [
    ("bitcoin", "BTC"),
    ("ethereum", "ETH"),
...
# These get populated at startup after verification
LARGE_CAPS  = []
SMALL_CAPS  = []
ALL_COINS   = []
...
def coinbase_jwt_headers(method: str, path: str) -> dict:
    """Generate JWT auth headers for Coinbase Advanced Trade API."""
    ...
    try:
        ...
        private_key = COINBASE_SECRET.replace("\\n", "\n")
        token = pyjwt.encode(
            payload,
            private_key,
            algorithm="ES256",
...
def coinbase_market_buy(symbol: str, size_usd: float):
    """Place a market buy on Coinbase using JWT auth."""
    try:
        import uuid
        ...
        headers = coinbase_jwt_headers("POST", path)
        if not headers:
            return None
...
def coinbase_market_sell(symbol: str, size_usd: float, price: float):
    """Place a market sell on Coinbase using JWT auth."""
    try:
        import uuid
        product_id = f"{symbol}-USDC"
        order_id   = str(uuid.uuid4())
...
verified_pairs = {}   # symbol -> {coinbase: bool, kraken: bool}

def verify_pair(symbol: str) -> bool:
    """Check if coin trades on both exchanges before attempting arb."""
    if symbol in verified_pairs:
        return verified_pairs[symbol]
    ...
    try:
        resp = requests.get(
            f"https://api.kraken.com/0/public/AssetPairs",
            params={"pair": pair}, timeout=5
        )
        result = resp.json().get("result", {})
        kraken_ok = bool(result) and "error" not in resp.json().get("error", [])
    except:
        pass

    verified_pairs[symbol] = kraken_ok
    if not kraken_ok:
        print(f"  ⚠ {symbol} not available on Kraken — removing from scan")
    return kraken_ok
    ...
    while True:
        try:
            ...
            for cg_id, symbol in ALL_COINS:
                ...
                if best:
                    # Verify coin trades on both exchanges before attempting
                    if not verify_pair(symbol):
                        continue
...
def discover_verified_pairs():
    """
    Test every coin against both Coinbase and Kraken APIs.
    Builds verified list automatically — no manual guessing.
    """
    global LARGE_CAPS, SMALL_CAPS, ALL_COINS

    print("\n🔍 Auto-discovering verified pairs...")
    print(f"  Testing {len(ALL_POSSIBLE_COINS)} coins on both exchanges...\n")
    ...
    # Get all Kraken USD pairs in one call
    ...
    try:
        ...
        for pair_name, pair_data in pairs.items():
            ...
            if quote in ("ZUSD", "USD") and not pair_name.endswith(".d"):
                coin = base.lstrip("X").lstrip("Z")
                if coin == "XBT": coin = "BTC"
                kraken_available.add(coin)
        print(f"  Kraken: {len(kraken_available)} USD pairs found")
    except Exception as e:
        print(f"  ⚠ Kraken API error: {e}")

    # Get all Coinbase USD/USDC pairs in one call
    ...
    try:
        ...
        for p in products:
            ...
            parts = pid.split("-")
            if len(parts) == 2 and parts[1] in ("USD", "USDC"):
                coinbase_available.add(parts[0])
        print(f"  Coinbase: {len(coinbase_available)} USD/USDC pairs found")
    except Exception as e:
        print(f"  ⚠ Coinbase API error: {e}")

    # Cross-reference
    for cg_id, symbol in ALL_POSSIBLE_COINS:
        on_kraken   = symbol in kraken_available
        on_coinbase = symbol in coinbase_available

        if on_kraken and on_coinbase:
            verified.append((cg_id, symbol))
        elif on_coinbase and not on_kraken:
            coinbase_only.append(symbol)
        elif on_kraken and not on_coinbase:
            kraken_only.append(symbol)
        else:
            neither.append(symbol)
    ...
    print(f"\n  ✅ Verified on BOTH: {[s for _, s in verified]}")
    print(f"  ⚠ Coinbase only: {coinbase_only}")
    print(f"  ⚠ Kraken only: {kraken_only}")
    print(f"  ❌ Neither: {neither}")
    print(f"\n  → Scanning {len(ALL_COINS)} verified pairs for arb\n")
...
""")
    ...
    # Auto-discover verified pairs on both exchanges
    num_verified = discover_verified_pairs()

    send_telegram(
        f"💎 *Coinbase Alpha Suite LIVE*\n\n"
        f"Mode: *{mode}*\n\n"
        f"🔍 Auto-verified: *{num_verified} coins* on both exchanges\n"
    ...
    while True:
        ...
        print(f"  ♻ {len(alive)}/5 | positions: {pos_str} | arb P&L: ${daily_arb_profit:+.2f} | signal P&L: ${daily_pnl:+.2f} | verified: {len(ALL_COINS)} pairs")
        
        # Re-verify pairs every 6 hours to catch new listings
        reverify_counter += 1
        if reverify_counter >= 360:
            reverify_counter = 0
            print("  🔄 Re-verifying pairs...")
